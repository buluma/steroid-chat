...
export class FileProcessingService {
  ...
  async processFile(file: File): Promise<FileAttachment> {
    ...
    switch (extension) {
      case 'txt': case 'md': case 'log': case 'csv': case 'js': case 'ts': 
      case 'py': case 'html': case 'css': case 'json': case 'xml': 
      case 'yaml': case 'yml':
  ...
  private async processTextFile(file: File): Promise<FileAttachment> {
    ...
    return { id: crypto.randomUUID(), name: file.name, type, content, mimeType: file.type || 'text/plain', size: file.size };
  }

  private async processImageFile(file: File): Promise<FileAttachment> {
    const base64 = await this.fileToBase64(file);
    return { id: crypto.randomUUID(), name: file.name, type: 'image', content: base64, mimeType: file.type || 'image/jpeg', size: file.size };
  }
  ...
  private async processPdfFile(file: File): Promise<FileAttachment> {
    return {
      ...
      name: file.name,
      type: 'pdf',
      content: `[PDF File: ${file.name}]\n\nNote: For best results, ensure your selected model supports PDF/vision processing.`,
  ...
  getFileIcon(type: FileAttachment['type']): string {
    const icons: Record<FileAttachment['type'], string> = { text: '📄', json: '📋', image: '🖼️', pdf: '📕' };
    return icons[type] || '📁';
  }
